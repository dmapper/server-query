...
function checkCollWhitelist(coll) {
  var letPass = false;
  
  for (var i = 0; (i < COLLECTIONS_WHITELIST.length) && !letPass; i++) {
    letPass = patternToRegExp(COLLECTIONS_WHITELIST[i]).test(coll);
  }
...
function handleQuery(shareRequest, next) {
  ...
  if (checkCollWhitelist(collection)) return next();
